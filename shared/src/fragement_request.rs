...
impl FragmentRequestBuilder {
    ...
    pub fn build(self) -> Result<FragmentRequest, String> {
        let worker_name = self.worker_name.ok_or_else(|| "Worker name is missing".to_string())?;
        let max_work_load = self.max_work_load.ok_or_else(|| "Max work load is missing".to_string())?;
